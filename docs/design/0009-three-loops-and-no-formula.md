...
| M5 | **Telegram publisher.** Long-poll `getUpdates`; each message → `mention::read` → `Gate::admit` → dossier → roast → log → reply. Unset token ⇒ nothing (rule 8). Not a contest entry, not in the record. **Built 2026-09-05** as [`telegram.rs`](../../crates/radar-analyst/src/telegram.rs): its own log file is what keeps it out of the record; a second switch, `RADAR_TELEGRAM_PUBLISH=on`, keeps it silent until read | parser, gate, fact path, `Publisher` trait in [`publish.rs`](../../crates/radar-analyst/src/publish.rs) | nothing further; the env section is in `deploy/analyst.env.example` and `radar brief` counts the lane | `deploy/README.md` | a bot token from Josh via BotFather, five minutes. A **different** bot from the alert one |
...
| M4 | **"Seven days later."** Once a day: replies seven days old from the log, joined once against the store's outcomes on the creator-index timer's pattern, rendered as one post to X and Telegram. **Built 2026-09-05**: the join is `radar seven-days-later` on [`deploy/radar-seven-days.timer`](../../deploy/radar-seven-days.timer), the post is [`daily.rs`](../../crates/radar-analyst/src/daily.rs) at 12:00 UTC, under the same two checks | the reply log ([`log.rs`](../../crates/radar-analyst/src/log.rs)); the outcomes; the timer pattern in [`deploy/radar-creator-index.service`](../../deploy/radar-creator-index.service) | nothing further | — | the bot live for seven days. Day one has nothing to say and says so |
| M2 | **The weekly teardown post.** The winner's mint gets the full dossier as the week's top-level post; the vault, the prize and the transaction in the same post (C8). **Built 2026-09-05** as [`weekly.rs`](../../crates/radar-analyst/src/weekly.rs) over the record [`contest.rs`](../../crates/radar-analyst/src/contest.rs) closes: the result under 280 characters as the top-level post and the teardown as its reply, the winner named by the reply's URL rather than a handle (the log carries ids, not names) | the week-close job and the roaster | nothing further | — | M1 |
| M3 | **The hunter rank.** Status only. Rule below | the signals are on the sheet ([`sheet.rs`](../../crates/radar-roast/src/sheet.rs)) — **counted there as a type since 2026-09-05** (`Signal`, three variants, the strongest band read off the snapshot rather than named) and carried on every reply log entry; the tally is [`hunter.rs`](../../crates/radar-contest/src/hunter.rs); the per-summoner cap is in the gate and, since the same day, charged on admission so a burst cannot read the chain past it | the week-close job that feeds the log to the tally (plan 0006 item 6); a second tab on [`site/src/Leaderboard.tsx`](../../site/src/Leaderboard.tsx) | — | M1. The rule below is accepted by the same delegation as L1–L6, with its change policy |
