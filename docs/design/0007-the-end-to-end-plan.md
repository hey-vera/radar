...
| E5 | **Re-run `0007` weekly** as `0007` itself asks, watching the prior-coverage line. E2 did not create a timer of its own, so this needs one — or a second  on the creator-index unit, which is where the store is already being read |  |
